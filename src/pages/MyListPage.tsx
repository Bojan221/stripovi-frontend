...
import type { FavoriteComic } from "../types/FavoriteComic";
...
function MyListPage() {
  ...
  const {
    ...
    queryKey: ["myList", currentPage, publisher, hero, edition, search, perPage],
    queryFn: async () => {
      const res = await axiosPrivate.get(
        `/api/favoriteComics/getFavoriteComics?page=${currentPage}&publisher=${publisher}&hero=${hero}&edition=${edition}&search=${search}&limit=${perPage}`,
      );
      return {
        favorites: res.data.comics as FavoriteComic[],
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
              Lista <span className="text-orange-500">Omiljenih</span>
            </h1>
            {!isLoading && (
        ...
        {isLoading ? (
            ...
            {hasActiveFilters ? (
              ...
              <>
                ...
                <p className="text-gray-900 font-black text-xl mb-1">
                  Lista omiljenih je prazna
                </p>
                <p className="text-gray-400 text-sm mb-6">
                  Dodaj stripove u omiljene klikom na srce u popup meniju.
                </p>
                <Link
                  to="/comics"
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {favorites.map((fav) => (
                <ComicCard key={fav._id} comic={fav.comic} />
              ))}
            </div>
